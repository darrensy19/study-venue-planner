...
const SERVICE_CORE_FROM_MINUTES = 420; // 07:00 — maintained assumption
const SERVICE_CORE_UNTIL_MINUTES = 1290; // 21:30 — maintained assumption
const SERVICE_DAY_START_MINUTES = 240; // 04:00 — structural
...
/** service_date(t) = date(t - SERVICE_DAY_START_MINUTES). A night's
...
export function serviceDateFromAbs(abs) {
  return dateFromAbs(abs - SERVICE_DAY_START_MINUTES);
}
 ...
 * clock string lands in [SERVICE_DAY_START_MINUTES, 1680).
 */
export function normaliseEdge(hhmm) {
  ...
  const raw = hh * 60 + mm;
  return raw < SERVICE_DAY_START_MINUTES ? raw + MINUTES_PER_DAY : raw;
}
...
export function resolveReturnBound(
  ...
  if (clock >= SERVICE_CORE_FROM_MINUTES && clock <= SERVICE_CORE_UNTIL_MINUTES) {
  ...
  if (clock >= SERVICE_DAY_START_MINUTES && clock < SERVICE_CORE_FROM_MINUTES) {
...
function boundBindingMetrics(venue, holidays, arrivalAbs, travelMinutes, durationMinutes, closingBufferMinutes, returnOutcome) {
  ...
  return { usableMinutes, surplus, latestLeaveAt, bindingConstraint: combined.bindingConstraint, bindingLimitAbs };
}
...
export function resolveOverallFeasibilityAtArrivals(venue, holidays, params) {
  ...
  if (returnResult.tier === "unverified") {
    ...
    return {
      ...
      bindingLimitMid: hoursResult.bindingLimitMid,
  ...
  return {
    ...
    bindingLimitMid: midMetrics?.bindingLimitAbs,
...
export function resolveBound(
  ...
  return { kind: "resolved", usableMinutes, surplus, latestLeaveAt, bindingLimitAbs: closeAdjusted };
}
...
export function resolveFeasibilityAtArrivals(venue, holidays, params) {
  ...
  return {
    ...
    bindingLimitMid: midNone ? undefined : mid.bindingLimitAbs,
...
// --- Outbound-mirror transport (ARCH-003/ARCH-004, "Getting there") --------

/**
 * resolveOutboundService(venue, holidays, origin, mode, serviceDate)
 *   -> {kind:"missing"} | {kind:"malformed", reason} | {kind:"present", lo, hi}
 *
 * Mirrors resolveReturnService exactly in structure — same holiday ->
 * by_weekday -> default -> MISSING precedence, same PRESENT/MISSING/MALFORMED
 * result — reading outbound_transport[origin][mode] and the shared
 * holiday_return_policy (there is no holiday_outbound_policy) instead of
 * return_transport[destination][mode]. The sole runtime authority on whether
 * a specific record is usable; outboundAdmissible reads only this return
 * value, never outbound_transport or outbound_transport_status directly.
 */
export function resolveOutboundService(venue, holidays, origin, mode, serviceDate) {
  const block = venue.outbound_transport?.[origin]?.[mode];
  if (!block) return { kind: "missing" };
...
/**
 * outboundAdmissible(venue, holidays, origin, mode, leaveAtAbs, serviceDate)
 *   -> {kind:"pass", basis} | {kind:"excluded", reason, label}
 *
 * A total function, evaluated once — no dual bounds, unlike the return leg:
 * leave_at is the raw input, not a derived value carrying travel-band
 * uncertainty, so there is no pessimistic bound to resolve independently.
 * Precondition, established by the caller and not re-checked here:
 * access[origin][mode] is present — this function only ever answers WHEN
 * service runs for a mode already known to have a recorded route, never
 * WHETHER a route exists.
 *
 * Step order is load-bearing, mirroring resolveReturnBound:
 *   1. A schedule-free mode (walk, or cycle given the access precondition)
 *      settles it immediately — positive evidence, no timetable read.
 *   2. Inside the core service span, the timetable lookup is waived (the
 *      route is already established by the precondition).
 *   3. The pre-dawn gap is hard-filtered unconditionally, before any
 *      outbound_transport read.
 *   4. Only now is outbound_transport consulted, and only via
 *      resolveOutboundService, which performs its own precedence selection
 *      fresh for this (origin, mode, serviceDate) — outbound_transport_status
 *      is never read at any step; a malformed entry can only ever affect the
 *      one selected record it belongs to.
 *
 * The pessimistic edge for a last departure is the band's lower (lo) edge,
 * regardless of leg direction — "service actually ended sooner than the
 * midpoint suggests."
 */
export function outboundAdmissible(venue, holidays, origin, mode, leaveAtAbs, serviceDate) {
  if (SCHEDULE_FREE_MODES.has(mode)) {
    return { kind: "pass", basis: "schedule_free" };
  }

  const clock = leaveAtAbs - absMinutes(serviceDate, 0);

  if (clock >= SERVICE_CORE_FROM_MINUTES && clock <= SERVICE_CORE_UNTIL_MINUTES) {
    return { kind: "pass", basis: "core_span" };
  }
  if (clock >= SERVICE_DAY_START_MINUTES && clock < SERVICE_CORE_FROM_MINUTES) {
    return { kind: "excluded", reason: "pre_dawn_gap", label: "outbound_gap" };
  }

  const resolved = resolveOutboundService(venue, holidays, origin, mode, serviceDate);
  if (resolved.kind === "malformed") {
    return { kind: "excluded", reason: "invalid_metadata", label: "outbound_data_error" };
  }
  if (resolved.kind === "missing") {
    return { kind: "excluded", reason: "missing_data", label: "outbound_gap" };
  }
  if (leaveAtAbs > absMinutes(serviceDate, resolved.lo)) {
    return { kind: "excluded", reason: "after_last_departure", label: "outbound_gap" };
  }
  return { kind: "pass", basis: "last_departure" };
}

/**
 * validateOutboundTransport(venuesMeta) -> { [venueId]: { rollup, perOriginMode } }
 *
 * Diagnostics only — never consulted by ranking, at any granularity (unlike
 * validateReturnTransport, whose per-venue stamp does gate ranking). Walks
 * every reachable outbound_transport entry (default and every by_weekday key,
 * every origin, every mode) and reports per-(origin, mode), since a coarser
 * per-venue or per-origin/mode stamp would let a malformed record outside a
 * query's actual scope override that query's answer. The one and only
 * runtime check lives inside resolveOutboundService, which re-derives
 * validity fresh for the one entry a given query actually selects.
 */
export function validateOutboundTransport(venuesMeta) {
  const status = {};
  for (const venueId of Object.keys(venuesMeta)) {
    const perOriginMode = {};
    let anyInvalid = false;
    const outboundTransport = venuesMeta[venueId].outbound_transport;
    if (outboundTransport) {
      for (const origin of Object.keys(outboundTransport)) {
        perOriginMode[origin] = {};
        for (const mode of Object.keys(outboundTransport[origin])) {
          const block = outboundTransport[origin][mode];
          const failures = [];
          const entries = [["default", block.default], ...Object.entries(block.by_weekday ?? {})];
          for (const [dayKey, entry] of entries) {
            if (!entry || !entry.last_departure_band) continue; // MISSING
            const r = normaliseBand(entry.last_departure_band);
            if (r.kind === "malformed") failures.push(`${dayKey}: ${r.reason}`);
          }
          perOriginMode[origin][mode] = failures.length
            ? { state: "invalid", reason: failures.join("; ") }
            : { state: "ok" };
          if (failures.length) anyInvalid = true;
        }
      }
    }
    status[venueId] = { rollup: anyInvalid ? "invalid" : "ok", perOriginMode };
  }
  return status;
}
...
/**
 * deriveLatestLeaveAtState(latestLeaveAt, departureAbs) -> "future" | "past" |
 * "undetermined" | "closed_at_arrival" (plan.md's "Requested end vs
 * achievable end"). `latestLeaveAt` is `undefined` exactly when the midpoint
 * bound has no active period at all — there is no deadline to state, so that
 * case is `closed_at_arrival` rather than a comparison against `departureAbs`.
 * The tagged `"UNDETERMINED"` string (COVERED hours, unbounded return) is
 * `undetermined`; everything else is a real absolute minute, compared against
 * the selected departure to say whether the deadline is still ahead of it.
 */
export function deriveLatestLeaveAtState(latestLeaveAt, departureAbs) {
  if (latestLeaveAt === undefined) return "closed_at_arrival";
  if (latestLeaveAt === "UNDETERMINED") return "undetermined";
  return latestLeaveAt >= departureAbs ? "future" : "past";
}

const FRESHNESS_FIELD_NAMES = new Set(["hoursStatus", "histogramStatus"]);

/**
 * stripFreshnessFields(value) -> a deep copy of `value` with every
 * `hoursStatus` / `histogramStatus` occurrence deleted, at every depth,
 * including inside arrays and nested objects such as `planB` and
 * `bestAlternative` (plan.md's "The stale-data invariant is structural, not a
 * maintained field list"). Exported so its own non-vacuity can be tested
 * directly against a synthetic object — a projector that just returns `{}`
 * would satisfy a behavioural before/after comparison alone without actually
 * doing this.
 */
export function stripFreshnessFields(value) {
  if (Array.isArray(value)) return value.map(stripFreshnessFields);
  if (value !== null && typeof value === "object") {
    const out = {};
    for (const [key, v] of Object.entries(value)) {
      if (FRESHNESS_FIELD_NAMES.has(key)) continue;
      out[key] = stripFreshnessFields(v);
    }
    return out;
  }
  return value;
...
/**
 * resolveVenueNaming(venues) -> Map<venueId, {displayName, disambiguatedLabel}>
 *
 * plan.md's "Naming is resolved once, from the whole snapshot, and propagated
 * to every object that names a venue" — computed once per rankVenues() call
 * so candidates, removed/travelUnknown entries and the nested Plan B view all
 * read the same two fields rather than each reconstructing a name.
 * `displayName` falls back to `venue.id` when `name` is absent; the
 * venue-source registry's `resolved_name` fallback for that case is `BL-013`,
 * deferred — no Python-side merge yet puts `resolved_name` on the JS-visible
 * venue object. `disambiguatedLabel` adds `area` only when `displayName` is
 * not unique across the whole snapshot, so `area` is never printed twice on
 * a surface that already shows it on its own line.
 */
function resolveVenueNaming(venues) {
  const displayNameOf = (venue) => venue.name ?? venue.id;
  const counts = new Map();
  for (const venue of venues) {
    const n = displayNameOf(venue);
    counts.set(n, (counts.get(n) ?? 0) + 1);
  }
  const naming = new Map();
  for (const venue of venues) {
    const displayName = displayNameOf(venue);
    const disambiguatedLabel = counts.get(displayName) > 1 ? `${displayName} (${venue.area})` : displayName;
    naming.set(venue.id, { displayName, disambiguatedLabel });
  }
  return naming;
}
 ...
 * bestAlternative, refusalInstant, travelUnknown, removed, snapshotEmpty,
 * hardFilteredCount }`. Every candidate (in `groups`, `alternatives`, `planA`
 * and `bestAlternative` alike) additionally carries `displayName` /
 * `disambiguatedLabel` (from `resolveVenueNaming`), `hoursStatus` /
 * `histogramStatus` (from the venue's own source records — plan.md's
 * "Evidence freshness"), `achievableSessionEndMid` (`arrivalMidAbs +
 * usableMinutesMid`, `undefined` exactly when `usableMinutesMid` is),
 * `latestLeaveAtState` (`deriveLatestLeaveAtState`'s four-way discriminant),
 * and `bindingLimitMid` (the raw instant the binding constraint bites, distinct
 * from the derived `latestLeaveAt` departure deadline) — plan.md's "The
 * returned presentation shape". A non-null `planB` carries the same naming
 * and freshness fields plus `travelMinutesMid` (the fallback's own parsed
 * travel band, otherwise discarded after Plan B selection). `removed` and
 * `travelUnknown` entries carry `displayName` / `disambiguatedLabel` and the
 * two freshness fields too, wherever the removed venue's own records exist to
 * read them from — hard-filtered venues stay aggregate-only, per
 * `hardFilteredCount` below:
 ...
 *     by area").
 *   - `bestAlternative` — non-null only under `session_does_not_fit`: the
 *     first (best-ranked) `groups.shorter` candidate, named by the pipeline
 *     rather than left for `app.js` to pick — plan.md: "`bestAlternative` is
 *     named by the pipeline; the renderer only renders it."
 *   - `refusalInstant` — non-`undefined` only under `no_verified_return`: the
 *     request's own nominal session end (`departureAbs + durationMinutes`,
 *     travel excluded, since every candidate's return is unverified and there
 *     is no single venue to hang a per-candidate instant off). An absolute
 *     instant, never pre-stringified — plan.md's "Refusal instant, day-marked".
 *   - `travelUnknown` — venues with an explicit-`null` access entry: `[{
 *     venueId, displayName, disambiguatedLabel, hoursStatus, histogramStatus }]`.
 *   - `removed` — every unranked-removal row of the taxonomy table, each
 *     `{ venueId, displayName, disambiguatedLabel, hoursStatus, histogramStatus,
 *     reason, kind }`. Access-key-missing venues (hard-filtered) are absent
 *     even from here — plan.md: "not a candidate at all" — and are counted in
 *     `hardFilteredCount` instead.
...
export function rankVenues(snapshot, controls) {
  ...
  const naming = resolveVenueNaming(venues);
  ...
  for (const venue of venues) {
    const { displayName: name, disambiguatedLabel } = naming.get(venue.id);
    const hoursStatus = venue.hours?.status;
    const histogramStatus = venue.histogram?.status;

    if (invalidPreference.has(venue.id)) {
      removed.push({ venueId: venue.id, displayName: name, disambiguatedLabel, hoursStatus, histogramStatus, reason: invalidPreference.get(venue.id), kind: "preference_invalid" });
      continue;
    }
    ...
    if (!venue.hours || !venue.hours.regular_hours) {
      removed.push({ venueId: venue.id, displayName: name, disambiguatedLabel, hoursStatus, histogramStatus, reason: "hours data was never obtained for this venue", kind: "hours_source_failed" });
      continue;
    }
    if (venue.business_status !== "OPERATIONAL") {
      removed.push({ venueId: venue.id, displayName: name, disambiguatedLabel, hoursStatus, histogramStatus, reason: `business_status is ${venue.business_status}`, kind: "not_operational" });
      continue;
    }
    ...
    if (accessEntry == null) {
      travelUnknown.push({ venueId: venue.id, displayName: name, disambiguatedLabel, hoursStatus, histogramStatus });
      continue;
    }

    const travelBand = normaliseTravelBand(accessEntry.band);
    if (travelBand.kind !== "present") {
      removed.push({ venueId: venue.id, displayName: name, disambiguatedLabel, hoursStatus, histogramStatus, reason: `access band: ${travelBand.reason ?? "not measured"}`, kind: "access_band_invalid" });
      continue;
    }

    // Outbound admissibility (plan.md's "Getting there: outbound-mirror
    // transport" — a hard filter, run once the selected origin/mode/leave_at
    // and travel band are confirmed, before any hours or return-leg
    // evaluation). Excluded the same way a missing access entry is, but
    // visibly, per a removal notice naming both the exact internal reason and
    // the coarser user-facing label.
    const serviceDate = serviceDateFromAbs(departureAbs);
    const outbound = outboundAdmissible(venue, holidays, origin, resolvedMode, departureAbs, serviceDate);
    if (outbound.kind === "excluded") {
      const reasonText = {
        pre_dawn_gap: "outbound: pre-dawn gap — no timetable is modelled before the core service span",
        missing_data: "outbound_transport: no departure recorded for this mode from this origin on this date",
        invalid_metadata: "outbound_transport: the recorded departure band is malformed",
        after_last_departure: "outbound_transport: leave time is after the last recorded departure",
      }[outbound.reason];
      removed.push({ venueId: venue.id, displayName: name, disambiguatedLabel, hoursStatus, histogramStatus, reason: reasonText, kind: outbound.label });
      continue;
    }
    ...
    if (overall.tier === "invalid") {
      removed.push({ venueId: venue.id, displayName: name, disambiguatedLabel, hoursStatus, histogramStatus, reason: overall.reason, kind: "contradictory_hours" });
      continue;
    }
    if (overall.tier === "hours-unknown") {
      removed.push({ venueId: venue.id, displayName: name, disambiguatedLabel, hoursStatus, histogramStatus, reason: "hours could not be resolved for this arrival", kind: "hours_unknown" });
      continue;
    }
    ...
    // Plan B naming/freshness read from the same whole-snapshot map and the
    // same venue record every other surface reads — never reconstructed
    // (plan.md's "Naming is resolved once... propagated to every object that
    // names a venue"; "Plan B transfer time" — plan.md's "The returned
    // presentation shape").
    const planBFallbackVenue = planB ? venueById.get(planB.fallbackVenueId) : undefined;
    const planBView = planB && planB.result.strength !== "none" ? {
      ...
      travelMinutesMid: planB.travelMinutesMid,
      ...naming.get(planB.fallbackVenueId),
      hoursStatus: planBFallbackVenue?.hours?.status,
      histogramStatus: planBFallbackVenue?.histogram?.status,
    } : null;
    ...
    candidates.push({
      ...
      usableMinutesMid: overall.usableMinutesMid,
      achievableSessionEndMid: overall.usableMinutesMid === undefined ? undefined : arrivalMidAbs + overall.usableMinutesMid,
      latestLeaveAt: overall.latestLeaveAt,
      latestLeaveAtState: deriveLatestLeaveAtState(overall.latestLeaveAt, departureAbs),
      bindingLimitMid: overall.bindingLimitMid,
      metricsBasis: overall.metricsBasis,
  ...
  // plan.md's "shorter stays a visible degraded alternative": named by the
  // pipeline, not reconstructed by app.js from group order (which would make
  // the renderer choose what to recommend). Scoped to `session_does_not_fit`
  // per "The returned presentation shape" — the only resultState where
  // groups.ranked is empty and a `shorter` candidate can stand in for it.
  const bestAlternative = resultState === "session_does_not_fit" ? groups.shorter[0] ?? null : null;

  // plan.md's "Refusal instant, day-marked": no_verified_return has no single
  // candidate to hang an instant off — every candidate's return is
  // unverified, but each keeps its own hours-only achievable end. The one
  // venue-independent instant available is the request's own nominal end
  // (departure + duration, travel excluded), carried as an absolute instant
  // rather than pre-stringified so a session crossing midnight still carries
  // its day.
  const refusalInstant = resultState === "no_verified_return" ? departureAbs + durationMinutes : undefined;
