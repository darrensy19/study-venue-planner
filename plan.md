...
**Venue names and brands have been supplied** — 28 venues across three brands, in `data/venue_seeds.csv`. The probe scripts are written and verified against synthetic payloads. **The only thing left before Phase 0 can be run is a `GOOGLE_PLACES_API_KEY` and a `SERPAPI_KEY`; neither account exists yet.** Everything below is still Phase 0's job to produce.
...
- ~~**How many venues in total?**~~ **Answered: 24**, plus non-Starbucks venues still to be named. The consequences are recorded in `decisions.md` — a SerpApi ceiling of roughly nine refreshes a month on the free tier, and 24+ venues of hand-maintained meta whose *cross-venue ordinal ranks* are the part that will not scale. Both were open questions; neither is a blocker for Phase 0.
